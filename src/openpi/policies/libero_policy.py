...
class LiberoInputs(transforms.DataTransformFn):
    ...
    def __call__(self, data: dict) -> dict:
        ...
        state = transforms.pad_to_dim(data["observation/state"], self.action_dim) if not drop_state else np.zeros(self.action_dim)
        ...
        inputs = {
            "state": state,
            "image": {
                "base_0_rgb": base_image if not drop_image else np.zeros_like(base_image),
                "left_wrist_0_rgb": wrist_image if not drop_image else np.zeros_like(base_image),
        ...
        if "prompt" in data:
            inputs["prompt"] = data["prompt"] if not drop_prompt else ""
